...
train_df <- train_df %>% filter(ball_count == 3)
train_df <- train_df %>% filter(Pitch != "F")
train_df <- train_df %>% filter(Bat_hand == "L")
train_df$sub_fl <- as.logical(train_df$sub_fl)

test_df <- test_df %>%
  mutate(sub_fl = ifelse(code == 18, 1, 0))
unique(test_df$sub_fl)
test_df <- test_df %>% filter(ball_count == 3)
test_df <- test_df %>% filter(Pitch != "F")
test_df <- test_df %>% filter(Bat_hand == "L")
...
# ============================ MODELS ===============================
# Interaction Model: Pitch*Location - ACTUALLY BASE MODEL
int_model <- glm(sub_fl ~ Pitch+
                   LBSU_score+
                   #  OPP_score+
                   #  num_outs+
                   #  num_baserunners+
...
pr_test_glm <- pr_auc_glm(int_model, test_df_glm, split = "TEST",
                          model_name = "LHH Final (glm int_model)")
