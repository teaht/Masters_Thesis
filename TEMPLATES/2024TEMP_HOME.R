...
# is pitch type, and location, and speed.
# ------------------------------------------------------------------
library(pdftools)
library(tidyverse)
library(writexl)
...
# ------------------------------------------------------------------
# 2024 Season Automated play-by-play and pitches
# ------------------------------------------------------------------

# ------------------------------------------------------------------
# EDIT - Upload Files
# ------------------------------------------------------------------
pdf_text <- pdf_text("/Users/teahthies/Desktop/BoxPDF2024/PDF/csub1.pdf") #edit
...
merge_indices <- c(39)  # Indices where the next row should be merged
#.                -0, -1, -2, ....
...
#------------------------------------------------------------------
table <- table %>%
  filter(!(Hitter %in% c("CSU", "Long")))  # edit - opponent team name
...
# EDIT - get lineup
# ------------------------------------------------------------------
lineup <- read_excel("/Users/teahthies/Desktop/BoxPDF2024/lineup2024/csubline1.xlsx", sheet = 1)
...
# ------------------------------------------------------------------
event <- read_excel("/Users/teahthies/Desktop/DONE/BoxPDF2023/event.xlsx", sheet = 1)
...
table <- table %>%
  mutate(
    ...
    ball_count = as.numeric(sapply(numbers, function(x) if(length(x) >= 1) x[1] else NA)),  # First number
    strike_count = as.numeric(sapply(numbers, function(x) if(length(x) >= 2) x[2] else NA))  # Second number
  ) %>%
  mutate(
...
table <- table %>%
  ...
  { .[1, "Pitcher"] <- "Fernandez"; .[4, "Pitcher"] <- "L"; .[26, "Pitcher"] <- "R"; .[30, "Pitcher"] <- "L"; .[44, "Pitcher"] <- "Quinlan"; .} %>%
...
table <- table %>%
  mutate(Catcher = NA) %>% 
  # Assign the pitcher values to specific rows
  { .[1, "Catcher"] <- "Durazo"; .[4, "Catcher"] <- ""; .[44, "Catcher"] <- "Magadan"; . } %>%
  # Group by BatPitch and fill Pitcher down within each group
  group_by(BatPitch) %>%
...
table$opponent <- "CSUB"  
table$home_umpire <- "Tyler Barfuss"
table$time_of_day <- 15036               # military time
table$weather <- 67                      # degree F
table$outcome <- "Win"                  # Win, Loss
table$year <- "2024"                     # year
table$time_minutes <- 110               # game length in minutes 
table$attendence <- 313
table$home_vis <- "H"                    # V, H
table$uniform <- "white"          # white_pins, black, black_Y_LB, greypins, LBC
                                  # blackpins, white, blk_whiteP
...
write_xlsx(table, "/Users/teahthies/Desktop/BoxPDF2024/2024/csub1.xlsx")
