...
# EDIT - Upload Files
# ------------------------------------------------------------------
pdf_text <- pdf_text("/Users/teahthies/Desktop/BoxPDF2024/PDF/TEAM.pdf") #edit
...
merge_indices <- c(35,58)  # Indices where the next row should be merged
#.                -0, -1, -2, ....
...
#------------------------------------------------------------------
table <- table %>%
  filter(!(Hitter %in% c("TEAM1", "TEAM2")))  # edit - opponent team name
...
# EDIT - get lineup
# ------------------------------------------------------------------
lineup <- read_excel("/Users/teahthies/Desktop/BoxPDF2024/lineup2024/TEAM.xlsx", sheet = 1)
...
table <- table %>%
  ...
  { .[1, "Pitcher"] <- "HOMEPITCH"; .[6, "Pitcher"] <- "AWAYPITCH"; .[40, "Pitcher"] <- "L"; .} %>%
...
##---------------------------------------------------
## EDIT ALL - add general game info columns
##---------------------------------------------------
table$game_location <- "away"            # home, away, neutral
table$game_type <- "conference"      # conference, non-conference
table$opponent <- "CSF"  
table$home_umpire <- "Todd Foy"
table$time_of_day <- 1800               # military time
table$weather <- 73                      # degree F
table$outcome <- "Loss"                  # Win, Loss
table$year <- "2024"                     # year
table$time_minutes <- 135               # game length in minutes 
table$attendence <- 848
...
write_xlsx(table, "/Users/teahthies/Desktop/BoxPDF2024/2024/TEAM.xlsx")
