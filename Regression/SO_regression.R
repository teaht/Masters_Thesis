...
df <- df %>%
  mutate(sub_fl = ifelse(code == 4, 1, 0))
unique(df$sub_fl)
df_strike2 <- df %>% filter(strike_count == 2)
nrow(df_strike2)
df_strike2$sub_fl <- as.logical(df_strike2$sub_fl)
...
base_model_RHH <- glm(sub_fl ~ Pitch+Location+
                        line_up_pos+
                        LBSU_score+
                        OPP_score+
                        num_outs+
...
summary(base_model_RHH) # AIC 1725
...
pitch_model_RHH <- glm(sub_fl ~ Pitch+
                         ...
                         p_throws, 
                       data = df_right_sub_train, family = binomial)
summary(pitch_model_RHH) # AIC 1817
...
loc_model_RHH <- glm(sub_fl ~ Location+
                       ...
                       p_throws, 
                     data = df_right_sub_train, family = binomial)
summary(loc_model_RHH) # AIC 1741
...
int_model_RHH <- glm(sub_fl ~ Pitch*Location+
                       ...
                       p_throws, 
                     data = df_right_sub_train, family = binomial)
summary(int_model_RHH) # AIC 1647
...
test_pred <- ifelse(test_probs > 0.25, 1, 0)
...
# Linearity and Homoscedasticity (equal variance)
plot(int_model_RHH, which = 1)  # Residuals vs Fitted
# normality
plot(int_model_RHH, which = 2)
# independence of errors
library(car)
durbinWatsonTest(int_model_RHH)
#---------------------
# Test - Test Data
#---------------------
...
# show metrics
round(accuracy, 2)  # 0.60
round(precision, 2)  # 0.57
round(recall, 2)     # 0.32
round(f1_score, 2)    # 0.41
round(specificity, 2)   # 0.82
round(auc(roc_obj), 2)    # 0.60
...
#--------------------------------------
## BIG BASE MODEL: include all variable that do not give away outcome variable
base_model_LHH <- glm(sub_fl ~ Pitch+Location+
                        line_up_pos+
                        LBSU_score+
                        OPP_score+
                        num_outs+
...
summary(base_model_LHH) # AIC 817
...
pitch_model_LHH <- glm(sub_fl ~ Pitch+
                         ...
                         p_throws, 
                       data = df_left_sub_train, family = binomial)
summary(pitch_model_LHH) # AIC 952
...
round(specificity, 2)   # 0.87
round(auc(roc_obj), 2)    # 0.62
