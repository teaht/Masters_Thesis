...
cols_used <- c("strike_count", "sub_fl", "Inning", "Pitch", "Location", "line_up_pos", 
               "OPP_score", "num_baserunners", "p_throws", "LBSU_score",
...
# OOB estimate of  error rate: 21.07% (lower the better)
# Variable importance
...
round(recall, 2)       # 0.58
round(f1_score, 2)      # 0.70
round(specificity, 2)   # 0.79
...
# show metrics
round(accuracy, 2)     #0.43
round(precision, 2)    # 1
round(recall, 2)       # 0.39
round(f1_score, 2)      # 0.56
round(specificity, 2)   # 1
round(auc(roc_obj), 2)    # 0.68
