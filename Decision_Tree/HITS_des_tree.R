...
#--------------------------------------------------------------------
# Question 1: Hit vs what pitch count and location?
#--------------------------------------------------------------------
# Prepare Data
...
#--------------------------------------------------------------------
table(df2$sub_fl) # FALSE=436, TRUE=341
prop.table(table(df2$sub_fl)) # FALSE (90%), TRUE (9%)
# balance it
df_1 <- df2 %>% filter(sub_fl == 1) # Separate the two classes
df_0 <- df2 %>% filter(sub_fl == 0)
# randomly sample the 1's to match 0's
df_1_oversampled <- df_1 %>% sample_n(nrow(df_0), replace = TRUE)
# Combine the two balanced subsets
df_balanced_oversampled <- bind_rows(df_0, df_1_oversampled)
prop.table(table(df_balanced_oversampled$sub_fl))
# cant make synthetic samples because not all data is numeric
...
confusion <- table(Predicted = predicted_classes, Actual = df_clean_train$sub_fl)
print(confusion)
...
round(precision, 2)    # 0.66
round(recall, 2)       # 0.64
round(f1_score, 2)      # 0.65
round(specificity, 2)   # 0.66
...
# show metrics
round(accuracy, 2)     #0.53
round(precision, 2)    # 0.55
round(recall, 2)       # 0.55
round(f1_score, 2)      # 0.55
round(specificity, 2)   # 0.52
round(auc(roc_obj), 2)    # 0.53


#--------------------------------------
# LHH Base Model - Pitch
#--------------------------------------
# clean data for model 
cols_used <- c("strike_count", "sub_fl", "Inning", "Pitch", "Location", "line_up_pos", 
               "OPP_score", "num_baserunners", "ball_count", "p_throws", "LBSU_score",
...
round(precision, 2)    # 0.84
round(recall, 2)       # 0.63
round(f1_score, 2)      # 0.72
...
# show metrics
round(accuracy, 2)     #0.56
round(precision, 2)    # 0.64
round(recall, 2)       # 0.58
round(f1_score, 2)      # 0.61
round(specificity, 2)   # 0.53
round(auc(roc_obj), 2)    # 0.53
