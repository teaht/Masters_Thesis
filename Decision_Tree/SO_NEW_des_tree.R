...
nrow(df)
# make a strikeout_fl column
df <- df %>%
  mutate(strikeout_fl = ifelse(code == 4, 1, 0))
unique(df$strikeout_fl)
df_strike2 <- df %>% filter(strike_count == 2)
nrow(df_strike2)
df_strike2$strikeout_fl <- as.logical(df_strike2$strikeout_fl)
...
train_df_raw <- df_strike2[1:cutoff, ]
test_df      <- df_strike2[(cutoff + 1):n, ]
...
table(train_df_raw$strikeout_fl)
prop.table(table(train_df_raw$strikeout_fl))
table(test_df$strikeout_fl)
prop.table(table(test_df$strikeout_fl))

#-----------------------------------------
# 2) Balance ONLY the training data
#    (random oversampling of minority)
#-----------------------------------------
df_0 <- train_df_raw %>% filter(strikeout_fl == 0)
df_1 <- train_df_raw %>% filter(strikeout_fl == 1)
...
# sanity checks
nrow(train_df)
prop.table(table(train_df$strikeout_fl))
...
cols_used <- c("strike_count", "strikeout_fl", "Inning", "Pitch", "Location", "line_up_pos", 
...
# RHH Models
#-----------------------------------------------------------------
base_model <- rpart(strikeout_fl ~ Pitch+Location+
                      line_up_pos+
                      LBSU_score+
                      OPP_score+
                      num_outs+
...
#-----------------------------
test_probs <- predict(base_model, newdata = df_clean_train, type = "prob")
nrow(test_probs)
nrow(df_clean_train)
unique(df_clean_train$strikeout_fl)
predicted_classes <- ifelse(test_probs[, "TRUE"] >= 0.5, TRUE, FALSE)
confusion <- table(Predicted = predicted_classes, Actual = df_clean_train$strikeout_fl)
print(confusion)
...
# AUC
roc_obj <- roc(df_clean_train$strikeout_fl, test_probs[, 1])
plot(roc_obj)

# show metrics
round(accuracy, 2)     #0.67
round(precision, 2)    # 0.81
round(recall, 2)       # 0.66
round(f1_score, 2)      # 0.73
round(specificity, 2)   # 0.69
round(auc(roc_obj), 2)    # 0.69

#-----------------------------
# test model predictions
#-----------------------------
test_probs <- predict(base_model, newdata = df_clean_test, type = "prob")
nrow(test_probs)
nrow(df_clean_test)
predicted_classes <- ifelse(test_probs[, "TRUE"] >= 0.5, TRUE, FALSE)
confusion <- table(Predicted = predicted_classes, Actual = df_clean_test$strikeout_fl)
print(confusion)
...
# AUC
roc_obj <- roc(df_clean_test$strikeout_fl, test_probs[, 1])
plot(roc_obj)

# show metrics
round(accuracy, 2)     #0.68
round(precision, 2)    # 0.60
round(recall, 2)       # 0.73
round(f1_score, 2)      # 0.66
round(specificity, 2)   # 0.65
round(auc(roc_obj), 2)    # 0.72
...
reduced_model <- rpart(strikeout_fl ~ Pitch+Location+
                         line_up_pos+
                         LBSU_score+
                         OPP_score+
                         num_outs+
                         ...
                         home_vis+
                         p_throws, 
                       data = df_clean_train, method = "class",
                       control = rpart.control(cp = 0.03))

# Plot tree
rpart.plot(reduced_model, type = 3, extra = 104, fallen.leaves = TRUE)
title("Base Tree: 65% Accuracy") 

#-----------------------------
# Train model predictions
#-----------------------------
test_probs <- predict(reduced_model, newdata = df_clean_train, type = "prob")
nrow(test_probs)
nrow(df_clean_train)
predicted_classes <- ifelse(test_probs[, "TRUE"] >= 0.5, TRUE, FALSE)
confusion <- table(Predicted = predicted_classes, Actual = df_clean_train$strikeout_fl)
...
# show metrics
round(accuracy, 2)     #0.72
round(precision, 2)    # 0.59
round(recall, 2)       # 0.74
round(f1_score, 2)      # 0.65
round(specificity, 2)   # 0.71
round(auc(roc_obj), 2)    # 0.71

#-----------------------------
# test model predictions
#-----------------------------
test_probs <- predict(reduced_model, newdata = df_clean_test, type = "prob")
nrow(test_probs)
nrow(df_clean_test)
predicted_classes <- ifelse(test_probs[, "TRUE"] >= 0.5, TRUE, FALSE)
confusion <- table(Predicted = predicted_classes, Actual = df_clean_test$strikeout_fl)
...
# show metrics
round(accuracy, 2)     #0.52
round(precision, 2)    # 0.31
round(recall, 2)       # 0.44
round(f1_score, 2)      # 0.36
round(specificity, 2)   # 0.56
round(auc(roc_obj), 2)    # 0.45
